...
link:./cmd/cli/jobctl.md[Jobctl CLI] provides a command-line interface to interact with the gRPC API. The following methods are available:
...
The gRPC API uses protobuf messages for job definitions, arguments, and results. See the link:api/grpc.proto[grpc.proto] file for full schema and usage examples.
